...
// Build the training data set by appending the two language arrays.
...
// function (given input & weights, predicts the language) and 
...
let sigmoid t = 1.0/(1.0 + exp(-t))

// The prediction function takes all input features, multiplies them by weights
...
// we have the expected label and features. We call 'predict weights features' 
// to calculate the predicted label. Then we calculate the Euclidean 
// distance between the true label and the predicted label.
...
let newWeights = 
    Array.map2 (fun w g -> w - g * eta) initialWeights gradient  

// Assuming your training improved the weights, 
// the error should be getting smaller!
error trainingData initialWeights
...
// 2. Plot the error after training to see how long it takes until the weights
